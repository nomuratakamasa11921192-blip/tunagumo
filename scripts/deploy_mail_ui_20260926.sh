...
OLD=f593388f1acebf11cb20b1ae33d2eef913e975f6962241c7d2dab6b52d103b63
NEW=8c81090e9d6f50fdc2f90697f0bb82dc618554ece63aa1a8d2e3488eee704a15
EXPECTED_IMAGE=sha256:8bf74a697fbaad63b3d4a05b4dd0ea7da79e50fa12be3a64702cdabe4ae50bf5
RUNNING_IMAGE=sha256:90ab225e8826d758747ee0fc70a0a58ffb10424de5f491e348c173fc86f3e0b5
STAMP=$(date +%Y%m%d_%H%M%S)
BK="$STAGE/backup-$STAMP"
ROLLBACK="tsunagumo-mail-errors-rollback:$STAMP"
RELEASE="tsunagumo-mail-errors-ui:$STAMP"
...
atomic_write='import hashlib,os,pathlib,sys; p=pathlib.Path(sys.argv[1]); data=sys.stdin.buffer.read(); assert hashlib.sha256(data).hexdigest()==sys.argv[2]; t=p.with_name(".mail-errors-release.tmp"); t.write_bytes(data); t.chmod(0o644); os.replace(t,p)'
write_host() {
