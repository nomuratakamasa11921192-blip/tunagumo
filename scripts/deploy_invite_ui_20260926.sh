...
# Run from a private staging directory containing the Git-exported index.html.
set -Eeuo pipefail
umask 077
STAGE=$(cd "$(dirname "$0")" && pwd -P)
TARGET=/opt/tsunagumo/saas/frontend
OLD=8ffd42d457195eb69425b97eea9704f76fb41e2c6e67d15ce7e93117de807046
NEW=dfc58e662cba38094d47349c867210d7b95c3dce1f30f64deed9646bfef61fca
EXPECTED_IMAGE=sha256:90ab225e8826d758747ee0fc70a0a58ffb10424de5f491e348c173fc86f3e0b5
STAMP=$(date +%Y%m%d_%H%M%S)
BK="$STAGE/backup-$STAMP"
ROLLBACK="tsunagumo-invite-rollback:$STAMP"
RELEASE="tsunagumo-invite-ui:$STAMP"
...
[ "$(hash_live)" = "$OLD" ]
[ "$(docker inspect -f '{{.Image}}' docker-api-1)" = "$EXPECTED_IMAGE" ]
...
atomic_write='import hashlib,os,pathlib,sys; p=pathlib.Path(sys.argv[1]); data=sys.stdin.buffer.read(); assert hashlib.sha256(data).hexdigest()==sys.argv[2]; t=p.with_name(".invite-release.tmp"); t.write_bytes(data); t.chmod(0o644); os.replace(t,p)'
write_host() {
