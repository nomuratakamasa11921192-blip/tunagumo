...
突き合わせて修正した。それまでの実装は必須パラメータ(year、タイル座標)を
送っておらず、実際のAPIでは必ずエラーになる状態だった。
- XIT001 不動産価格(取引価格・成約価格): yearが必須、area/city/stationのどれか1つ。quarterで期間を絞る
...
2026-09-17、開発環境から実APIを確認済み。埼玉県の市区町村74件、春日部市の取引価格814件・
人口メッシュ897件を取得し、画面用の価格・人口要約まで検証した。他地域の網羅性は未検証。
"""
