...
現在の開発運用（2026-09-18）：**普段の開発はCodex・Claude CodeともローカルPC中心**。
最初に **[ローカルの開始手順](docs/local_start.md)**、次に[PCとVPSの同期手順](docs/local_vps_workflow.md)を読んでください。
Codex・Claude Codeの作業場所、ブランチ、ENV、引き継ぎはこちらを先に確認してください。
以下の初期配置手順は既存cloneではやり直さないでください。
